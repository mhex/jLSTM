...
public class LSTM {
    ...
    boolean[] initDropoutMask() {
    	...
    	int dropout = (int)Math.round(numBlocks * this.dropoutRate);
	...
	boolean compError() {
		...
		for (int i = firstOutputUnit, j = 0; i < numUnits; i++, j++) {
			...
			//System.out.println(i + " " + j + " " + activationNew[i] + " " + targets[j] + " " + error[j]);
			...
			//System.out.println(j + " " + activationNew[i] + " " + targets[j] + " " + error[j]);
	...
	private void compAUC(int[] rank, Integer[] rankSorted, int numSeqs) {
		...
		for (int i = 0; i < numSeqs; i++) {
			...
			//System.out.println(i + " " + aucVal + " " + this.numOutputUnits + " " + rankSorted[i] + " " + sense + " " + fpr);
	...
	public void timeforward() {
		...
		for (int i = 0; i < numUnits; i++) {
			activationOld[i] = activationNew[i];
		}
	...
	public void forwardPass(int element, boolean targ, boolean train) {
		...
		for (int u = 0; u < numBlocks; u++) {
			...
			for (int v = 0; v < blockSize[u]; v++) {
               ...
               activationNew[i] = activationCell[u][v];
				...
				//System.out.println(u + " " + i + " " + activationNew[i]);
		...
		if (targ) /* only if target for this input */ {
			...
			for (int k = firstOutputUnit; k < numUnits; k++) {
				...
				maxSum = (maxSum > activationNew[k]) ? maxSum : activationNew[k];
			...
			for (int k = firstOutputUnit; k < numUnits; k++) {
            	...
            	if (float.isNaN(tmp)) {
            		...
            		System.out.println("tmp exp " + k + " " + tmp +  " " + maxSum);
            		System.exit(1);
            	}
				 */
		...
		for (int k = firstOutputUnit, j = 0; k < numUnits; k++, j++) {
			...
			for (int u = 0; u < numBlocks; u++) {
				...
				for (int v = 0; v < blockSize[u]; v++) {
					...
					//System.out.println(k + " " + i + " " + errorSignalOutput[k] + " " + activationNew[i] + " " + dW[k][i]);
			...
			//System.out.println(k + " " + (i - 1) + " " + errorSignalOutput[k] + " " + activationNew[i - 1] + " " + dW[k][i - 1]);
		...
		for (int u = 0; u < numBlocks; u++) {
			...
			for (int v = 0; v < blockSize[u]; v++) {
				...
				for (int k = firstOutputUnit; k < numUnits; k++) {
				    ...
				    //System.out.println(i + " " + u + " "  + k + " " + weightMatrixLocal[k][i] + " " + errorSignalOutput[k] + " " + sum);
				...
				//System.out.println(i + " " + u + " "  + h[u][v] + " " + activationOutputGate[u] + " " + sum + " " + errorSignalMemoryState[u][v]);
		...
		for (int u = 0; u < numBlocks; u++) {
			...
			for (int j = biasUnit; j <= lastMemoryUnit; j++) {
                ...
                if (float.isNaN(sum)) {
            		...
            		System.out.println("sum bp ig " + i + " " + j + " " + errorSignalMemoryState[u][0] + " " + si[u][0][j]);
            		System.exit(1);
            	}
				 */
			...
			for (int v = 0; v < blockSize[u]; v++) {
				...
				for (int j = 0; j <= biasUnit; j++) {
					...
					//System.out.println(i + " " + j + " " + dW[i][j] + " " + errorSignalMemoryState[u][v] + " " + sc[u][v][j]);
		...
		for (int u = 0; u < numBlocks; u++) {
			...
			for (int v = 0; v < blockSize[u]; v++) {
				...
				for (int j = 0; j < numInputsLocalCoding; j++) {
					...
					//System.out.println("sc: " + j + " " + inputIndex[element][j] + " " + tmp3);
				...
				/*
                if (float.isNaN(si[u][v][biasUnit])) {

            		System.out.println("dv bias" + biasUnit + " " + u + " tmp2: " + tmp2 + " tmp3: " + tmp3 + " tmp1: " + tmp1);
            		System.exit(1);
		}
				 */
				...
				for (int j = biasUnit + 1; j <= lastMemoryUnit; j++) {
					...
					/*
                    if (float.isNaN(si[u][v][j])) {

            		System.out.println("dv " + j + " " + u + " " + activationOld[j] + " tmp2: " + tmp2 + " tmp3: " + tmp3 + " tmp1: " + tmp1);
            		System.exit(1);
		    }
					 */
